 ...
 * <b>NOTE: </b> Do not make heavy use of this class. It has an API key which has a limited number of requests per day,
 * which will get exceeded under heavy use (and this class makes heavy use of the API).
 *
 * @author Juan Li Puma
 */
...
//        me.downloadAllVideos();
    ...
    public void downloadAllCompanies() {
        try {
            paginate("companies/?fields=*&order=name%3Adesc", new SqlConsumer(companiesFile) {
                @Override
...
//                        System.out.println(platformQueries.toString());
            ...
            gamePlatformsFile.write(") AS val (game_id, id, release_date)\n" +
                    "INNER JOIN games ON val.game_id = games.id\n" +
    ...
    public void downloadAllVideos() {
        try {
            ...
            paginate("games/?fields=*&order=name%3Aasc", new SqlConsumer() {
                ...
                public void accept(JSONObject game) {
                    if(game.has("videos")) {
                        long gameId = game.getLong("id");
                        JSONArray videos = game.getJSONArray("videos");
                        for (int i = 0; i < videos.length(); i++) {
                            try {
                                JSONObject video = videos.getJSONObject(i);
                                if(!video.has("video_id")) {
                                    System.err.println("Video for game #" + gameId + " has no ID, skipping");
                                    continue;
                                }
                                String videoId = video.getString("video_id");
                                String videoName = video.has("name") ? escapeQuotesToPostgres(video.getString("name")) : "Video";
                                String query = "\t("+gameId+", '"+videoId+"', '"+videoName+"'),\n";
                                System.out.print(query);
                                gameVideosFile.write(query);
                            } catch (Exception e) {
                                e.printStackTrace();
                                System.exit(1);
                            }
            ...
            gameVideosFile.write(") AS val (game_id, video_id, video_name)\n" +
                    "INNER JOIN games ON val.game_id = games.id\n" +
                    "ON CONFLICT DO NOTHING;\n" +
                    "\n" +
                    "COMMIT;\n");
            gameVideosFile.close();
        } catch (UnirestException | IOException e) {
            e.printStackTrace();
        }
    }
...
//                    throw new RuntimeException();
    ...
    private String getISODateString(long timestamp) {
        return Instant.ofEpochMilli(timestamp).atZone(ZoneId.systemDefault()).toLocalDate().format(ISO8601Formatter);
    }
